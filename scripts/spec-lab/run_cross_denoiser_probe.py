#!/usr/bin/env python3
"""Drive the cross-denoiser bias-detector probe on ONE money-safe RunPod GPU.

WHAT IT MEASURES (pod/exp_cross_denoiser_probe.py): whether per-tile
OIDN-vs-OptiX disagreement D on the SAME same-seed render localizes the
DENOISER-BIAS-limited tiles that keep the integrated 4K receipt below the
strict delivery gate — the selector candidate RUN 4 pointed to after two-draft
variance divergence measured selector_recall = 0.0. One classroom frame at
3840x2160: two RUN 3-config anchors (OIDN / OptiX, same seed) + one 4096-spp
reference; recall@1/4/12, Spearman(D, E_oidn) and the full 64-tile arrays land
in the ledger. Estimated spend: ~$0.75-1.20 (about 35-45 GPU-minutes on an
A100; more if the H100 rung pays the one-time sm_90 probe JIT).

MONEY-SAFETY (the adversarially-verified standard of
run_integrated_production_benchmark.py):
  * ONE driver at a time — shared .tracked_pods.json means concurrent drivers
    kill each other's pods; preflight REFUSES to start if any tracked or live
    pod exists.
  * register_cleanup() wires teardown to every exit path before any provision.
  * arm_remote_watchdog() is the FIRST action on the pod (TTL well above the
    run budget) so an orphan self-terminates even if this process dies.
  ...
  * The long render runs DETACHED on the pod (runpod.ssh_detached) so a peer
    reset cannot kill it; gpu_probe_timeout_s=1500 gives the sm_90 first-render
    JIT headroom.
...
LEDGER = REPO / "docs/speed-lane-reports/spec-lab/cross_denoiser_probe_ledger.jsonl"
POD_DISK_GB = 120
...
def log(message: str) -> None:
    print(f"[cross-denoiser-probe {time.strftime('%H:%M:%S')}] {message}", flush=True)


def remote_probe(pod: dict[str, Any], config: dict[str, Any], timeout: int) -> dict[str, Any]:
    payload = json.dumps(config).replace("'", "'\\''")
    cmd = "python3 pod/exp_cross_denoiser_probe.py '" + payload + "'"
    # The renders total ~35-45 min — the exact run-length band where a single
    # synchronous SSH has already died to a peer reset (2026-07-09 incident).
    # Detached-on-pod + short reconnecting polls; (rc, stdout, stderr_tail) keeps
    # the final-JSON-line contract.
    rc, out, err = runpod.ssh_detached(
        pod, cmd,
        workdir="/root/spec-lab",
        tag="cross-denoiser-probe",
        timeout_s=timeout,
        poll_every=20,
    )
...
def main() -> None:
    ...
    parser.add_argument("--job-id", default="cx-cross-denoiser-probe")
    ...
    config = {
        ...
        # sm_90 first-render JIT headroom (same rationale + value as the
        # integrated driver; the probe absorbs the one-time JIT, real renders
        # then hit the warmed per-user CUDA cache).
    ...
    append_jsonl(LEDGER, {"event": "cross_denoiser_probe_preflight", "preflight": preflight})
    ...
    try:
        ...
        append_jsonl(LEDGER, {"event": "cross_denoiser_probe_provision_pruned",
    ...
    try:
        ...
        append_jsonl(LEDGER, {
            "event": "cross_denoiser_probe_result",
            "metrics": metrics,
            "config": config,
        ...
        log(f"recall@1={metrics.get('recall_at_1')} recall@4={metrics.get('recall_at_4')} "
            f"recall@12={metrics.get('recall_at_12')} "
            f"spearman={metrics.get('spearman_D_vs_E_oidn')}")
