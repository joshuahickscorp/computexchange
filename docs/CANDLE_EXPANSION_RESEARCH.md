...
| L4 | **KvCacheSlot bounds/grow guard** | Fixed 4096 buffer, NO `cur_len+seq_len<=MAX_SEQ_LEN` check before `slice_set`; >4096 context fails opaquely (not a regression — pre-existing ceiling; rotary `narrow` errors first, slice_set has its own bail) | `quantized_llama_batched.rs:33,95-114`; upstream candle-nn `kv_cache.rs:69-75` grows | Latent correctness; bounds product to <4096 silently | None for a guard (same offsets/bytes) | Test: prefill+decode past 4096 returns typed RunError or clean grow == cat | **ship-now** (cheap typed error) |
...
- [ ] **KvCacheSlot bounds/grow** returns a typed error or grows past 4096 (no opaque slice_set failure).
