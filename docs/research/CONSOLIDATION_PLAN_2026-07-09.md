...
| render (selector probe 2 — cross-denoiser) | 4K classroom frame 1, OIDN-vs-OptiX same-seed per-tile disagreement D vs true error E_oidn, H100, ~$1.3, 26 min | recall@1=0.0, @4=0.0, @12=0.083; Spearman(D, E)=**-0.02** | MEASURED — SECOND decisive selector negative. The denoisers AGREE (max D=0.007 vs worst-tile error 0.091) and are BOTH ~equally wrong on the failing tiles (E_oidn 0.0905 / E_optix 0.0909); the D-map (row 3, mid-frame) is orthogonal to the error-map (corners [0,7]/[1,7]/[7,0]). Cross-denoiser disagreement does NOT localize the failure — the strict-gate miss is a SHARED denoiser bias on hard motion-reveal edge/corner content, not variance (RUN 4) and not denoiser-specific | selector dead |

### Selector question — status after two decisive negatives (2026-07-10)

Two reference-free tile selectors are now measured-dead against the strict-delivery gap (worst-tile
0.91 -> 0.95): (1) two-draft VARIANCE divergence (RUN 4, recall 0.0) and (2) cross-denoiser
DISAGREEMENT (recall ~0.08, Spearman -0.02). Both proved the failing tiles are NOT variance-limited
and NOT denoiser-specific — they are a SHARED denoiser bias on hard content, concentrated at
motion-reveal frame edges/corners. Untested principled signals remaining (one exhaustive multi-
selector probe planned, ~$1.3, all scored from ONE render): (a) Cycles' ADAPTIVE SAMPLE-COUNT pass
(the renderer's own per-region convergence-difficulty map — the strongest untested candidate: if the
corners are under-converged it localizes them; if converged-but-biased it confirms the ceiling),
(b) DENOISER RESIDUAL |noisy - denoised| (where the denoiser did the most work = most likely to err),
(c) content high-frequency/gradient energy, (d) AOV (normal/albedo) edge density. If NONE clears a
useful recall, the honest conclusion is definitive: **reference-free tile-repair cannot reach strict
delivery on this failure mode; 5.56x @ global 0.985 / worst-tile 0.91 (clears the published 0.95-tier)
is the honest ceiling of this lever**, and strict delivery would require a better DENOISER (re-opening
the owned-denoiser thread with a far sharper target: beat OIDN on its shared-bias edge tiles only) or
a higher-sample anchor (which costs the multiplier). The product number stands either way.
