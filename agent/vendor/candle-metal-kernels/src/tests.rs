...
const CX_Q4K_BLOCK_VALUES: usize = 256;
const CX_Q4K_BLOCK_BYTES: usize = 144;

fn q4k_test_weights(n: usize, k: usize) -> Vec<u8> {
    assert!(k.is_multiple_of(CX_Q4K_BLOCK_VALUES));
    let blocks_per_row = k / CX_Q4K_BLOCK_VALUES;
    let mut packed = vec![0u8; n * blocks_per_row * CX_Q4K_BLOCK_BYTES];
    for row in 0..n {
        for block in 0..blocks_per_row {
            let base = (row * blocks_per_row + block) * CX_Q4K_BLOCK_BYTES;
            let d = f16::from_f32(0.0025 + (block % 5) as f32 * 0.0001)
                .to_bits()
                .to_le_bytes();
            let dmin = f16::from_f32(0.00125 + (row % 7) as f32 * 0.00005)
                .to_bits()
                .to_le_bytes();
            packed[base..base + 2].copy_from_slice(&d);
            packed[base + 2..base + 4].copy_from_slice(&dmin);
            for i in 0..12 {
                packed[base + 4 + i] = ((row * 11 + block * 7 + i * 13) & 0xff) as u8;
            }
            for i in 0..128 {
                packed[base + 16 + i] = ((row * 17 + block * 29 + i * 31 + i * i) & 0xff) as u8;
            }
        }
    }
    packed
...
fn q4k_test_input(m: usize, k: usize) -> Vec<f32> {
    (0..m * k)
        .map(|i| {
            let centered = (i % 61) as f32 - 30.0;
            centered * 0.003 + ((i / k) as f32 * 0.0007)
        })
        .collect()
}
...
fn q4k_metal_case(device: &Device, m: usize, n: usize, k: usize) -> Q4KMetalCase {
    q4k_metal_batched_case(device, 1, m, n, k)
}

fn q4k_metal_batched_case(
    device: &Device,
    batch: usize,
    m: usize,
    n: usize,
    ...
    let row_bytes = k / CX_Q4K_BLOCK_VALUES * CX_Q4K_BLOCK_BYTES;
    let rows = batch * m;
    Q4KMetalCase {
        weights: new_buffer(device, &q4k_test_weights(n, k)),
        input: new_buffer(device, &q4k_test_input(rows, k)),
        output: device
            .new_buffer(rows * n * std::mem::size_of::<f32>(), RESOURCE_OPTIONS)
            .unwrap(),
        src0_shape: [1, 1, n, k],
        src0_stride: [n * row_bytes, n * row_bytes, row_bytes, 0],
        src1_shape: [1, batch, m, k],
        src1_stride: [rows * k * 4, m * k * 4, k * 4, 4],
        dst_shape: [1, batch, m, n],
    }
}
...
fn q4k_dispatch(
    ...
    match path {
        Q4KTestPath::StockMm | Q4KTestPath::SplitK | Q4KTestPath::SkinnyAuto => {
            crate::kernels::quantized::call_quantized_matmul_mm_t_for_test(
                device,
                &command_buffer,
                kernels,
                GgmlDType::Q4K,
                ...
                0,
                &case.output,
                matches!(path, Q4KTestPath::SplitK),
                matches!(path, Q4KTestPath::SkinnyAuto),
            )
            .unwrap();
        }
        Q4KTestPath::BatchedMv => {
            let [_, _, m, k] = case.src1_shape;
            let [_, _, n, _] = case.src0_shape;
...
fn assert_q4k_close(
    ...
    assert_eq!(stock.len(), optimized.len());
    let mut worst = (0usize, 0.0f32, 0.0f32, 0.0f32);
    for (index, (&a, &b)) in stock.iter().zip(optimized).enumerate() {
        assert!(
            a.is_finite() && b.is_finite(),
            "non-finite Q4_K output at {index}: {a} {b}"
        );
        let difference = (a - b).abs();
        if difference > worst.1 {
            worst = (index, difference, a, b);
        }
        let tolerance = absolute_tolerance + relative_tolerance * a.abs().max(b.abs());
        assert!(
            difference <= tolerance,
            "Q4_K {path} parity failed for {shape:?} at {index}: stock={a}, optimized={b}, diff={difference}, tolerance={tolerance}"
        );
    }
    eprintln!(
        "Q4_K {path} parity {shape:?}: worst index={} abs_diff={} stock={} optimized={}",
        worst.0, worst.1, worst.2, worst.3
    );
}

#[test]
fn q4k_skinny_gpu_parity_m2_through_m5_and_tail_n() {
    let device = device();
    let kernels = Kernels::new();
    let cases = [
        (2, 65, 512),
    ...
    for (m, n, k) in cases {
        let stock = q4k_metal_case(&device, m, n, k);
        q4k_dispatch(&device, &kernels, &stock, Q4KTestPath::StockMm);
        let stock_values = read_to_vec::<f32>(&stock.output, m * n);

        let split = q4k_metal_case(&device, m, n, k);
        q4k_dispatch(&device, &kernels, &split, Q4KTestPath::SplitK);
        let split_values = read_to_vec::<f32>(&split.output, m * n);
        assert_q4k_close(
            "split-K",
            &stock_values,
            &split_values,
        ...
        let skinny = q4k_metal_case(&device, m, n, k);
        q4k_dispatch(&device, &kernels, &skinny, Q4KTestPath::SkinnyAuto);
        let skinny_values = read_to_vec::<f32>(&skinny.output, m * n);
        assert_q4k_close(
            "skinny",
            &stock_values,
            &skinny_values,
        ...
        // Candle's upstream Q4_K MV kernel writes four output rows per
        // threadgroup without a tail predicate. Production dimensions are
        // divisible by four; the runtime skinny-M route must fall back for a
        // tail N rather than exposing that upstream OOB behavior.
        if n.is_multiple_of(4) {
            let batched_mv = q4k_metal_case(&device, m, n, k);
            q4k_dispatch(&device, &kernels, &batched_mv, Q4KTestPath::BatchedMv);
            let batched_mv_values = read_to_vec::<f32>(&batched_mv.output, m * n);
            assert_q4k_close(
                "batched-MV",
                &stock_values,
                &batched_mv_values,
...
#[test]
fn q4k_skinny_flattens_contiguous_rank3_cohorts() {
    let device = device();
    let kernels = Kernels::new();
    let cases = [
        (2, 2, 65, 512),
    ...
    for (batch, span, n, k) in cases {
        let rows = batch * span;
        let stock = q4k_metal_batched_case(&device, batch, span, n, k);
        q4k_dispatch(&device, &kernels, &stock, Q4KTestPath::StockMm);
        let stock_values = read_to_vec::<f32>(&stock.output, rows * n);

        let skinny = q4k_metal_batched_case(&device, batch, span, n, k);
        q4k_dispatch(&device, &kernels, &skinny, Q4KTestPath::SkinnyAuto);
        let skinny_values = read_to_vec::<f32>(&skinny.output, rows * n);
        assert_q4k_close(
            "flattened-rank3-skinny",
            &stock_values,
            &skinny_values,
            (rows, n, k),
            0.005,
...
#[test]
#[ignore = "manual performance gate; run on the target Apple verification class"]
fn q4k_skinny_benchmark_gate() {
    let device = device();
    let kernels = Kernels::new();
    let required = std::env::var("CX_Q4K_SKINNY_MIN_SPEEDUP")
        .or_else(|_| std::env::var("CX_Q4K_SPLITK_MIN_SPEEDUP"))
        .ok()
        .and_then(|value| value.parse::<f64>().ok())
        .filter(|value| value.is_finite() && *value >= 1.0)
        .unwrap_or(1.0);
    let shapes = [
        ("attention-kv", 4, 512, 2_048),
        ("hidden-m2", 2, 2_048, 2_048),
        ("hidden-m3", 3, 2_048, 2_048),
        ("hidden-m4", 4, 2_048, 2_048),
        ("hidden-m5", 5, 2_048, 2_048),
        ("mlp-up", 4, 8_192, 2_048),
        ("mlp-down", 4, 2_048, 8_192),
    ];

    for (label, m, n, k) in shapes {
        let case = q4k_metal_case(&device, m, n, k);
        // Compile both pipelines and stabilize one dispatch before measuring.
        for path in [
            Q4KTestPath::StockMm,
            Q4KTestPath::SplitK,
            Q4KTestPath::SkinnyAuto,
            Q4KTestPath::BatchedMv,
        ] {
            q4k_dispatch(&device, &kernels, &case, path);
        }

        let mut stock = Vec::with_capacity(15);
        let mut split = Vec::with_capacity(15);
        let mut skinny = Vec::with_capacity(15);
        let mut batched_mv = Vec::with_capacity(15);
        for _ in 0..15 {
            stock.push(q4k_dispatch(&device, &kernels, &case, Q4KTestPath::StockMm));
            split.push(q4k_dispatch(&device, &kernels, &case, Q4KTestPath::SplitK));
            skinny.push(q4k_dispatch(
                &device,
                &kernels,
                &case,
                Q4KTestPath::SkinnyAuto,
            ));
            batched_mv.push(q4k_dispatch(
                &device,
                &kernels,
                &case,
                Q4KTestPath::BatchedMv,
        ...
        stock.sort_unstable();
        split.sort_unstable();
        skinny.sort_unstable();
        batched_mv.sort_unstable();
        let stock_median = stock[stock.len() / 2].as_secs_f64();
        let split_median = split[split.len() / 2].as_secs_f64();
        let skinny_median = skinny[skinny.len() / 2].as_secs_f64();
        let batched_mv_median = batched_mv[batched_mv.len() / 2].as_secs_f64();
        let speedup = stock_median / split_median;
        let skinny_speedup = stock_median / skinny_median;
        let batched_mv_speedup = stock_median / batched_mv_median;
        eprintln!(
            "Q4_K {label} M={m} N={n} K={k}: stock={:.3}ms split-K={:.3}ms ({:.3}x) skinny={:.3}ms ({:.3}x) batched-MV={:.3}ms ({:.3}x) required={required:.3}x",
            stock_median * 1_000.0,
            split_median * 1_000.0,
            speedup,
            skinny_median * 1_000.0,
            skinny_speedup,
            batched_mv_median * 1_000.0,
            batched_mv_speedup,
        );
        assert!(
            skinny_speedup >= required,
            "Q4_K skinny-auto benchmark gate failed for {label}: {skinny_speedup:.3}x < {required:.3}x"
        );
    }
}

#[test]
#[ignore = "manual performance gate for flattened speculative cohorts"]
fn q4k_flattened_batch_benchmark_gate() {
    let device = device();
    let kernels = Kernels::new();
    let required = std::env::var("CX_Q4K_FLAT_BATCH_MIN_SPEEDUP")
        .ok()
        .and_then(|value| value.parse::<f64>().ok())
        .filter(|value| value.is_finite() && *value >= 1.0)
        .unwrap_or(1.0);
    let shapes = [
        ("mlp-down", 2, 2, 2_048, 8_192),
        ("output", 2, 2, 128_256, 2_048),
    ];

    for (label, batch, span, n, k) in shapes {
        let case = q4k_metal_batched_case(&device, batch, span, n, k);
        for path in [Q4KTestPath::StockMm, Q4KTestPath::SkinnyAuto] {
            q4k_dispatch(&device, &kernels, &case, path);
        }

        let mut stock = Vec::with_capacity(30);
        let mut flattened = Vec::with_capacity(30);
        for rep in 0..30 {
            let (first, second) = if rep % 2 == 0 {
                (Q4KTestPath::StockMm, Q4KTestPath::SkinnyAuto)
            } else {
                (Q4KTestPath::SkinnyAuto, Q4KTestPath::StockMm)
            };
            let first_elapsed = q4k_dispatch(&device, &kernels, &case, first);
            let second_elapsed = q4k_dispatch(&device, &kernels, &case, second);
            match first {
                Q4KTestPath::StockMm => stock.push(first_elapsed),
                Q4KTestPath::SkinnyAuto => flattened.push(first_elapsed),
                _ => unreachable!(),
            }
            match second {
                Q4KTestPath::StockMm => stock.push(second_elapsed),
                Q4KTestPath::SkinnyAuto => flattened.push(second_elapsed),
                _ => unreachable!(),
            }
        }
        stock.sort_unstable();
        ...
        let stock_median = stock[stock.len() / 2].as_secs_f64();
        let flattened_median = flattened[flattened.len() / 2].as_secs_f64();
        let speedup = stock_median / flattened_median;
        eprintln!(
            "Q4_K flattened {label} B={batch} span={span} rows={} N={n} K={k}: stock={:.3}ms skinny={:.3}ms ({speedup:.3}x), required={required:.3}x",
            batch * span,
            stock_median * 1_000.0,
            flattened_median * 1_000.0,
        );
        assert!(
            speedup >= required,
            "Q4_K flattened benchmark gate failed for {label}: {speedup:.3}x < {required:.3}x"
        );
    }
}
